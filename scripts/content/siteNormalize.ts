...
import { isCodeManagedEntry } from "../../src/cms/contentOwnership";

export function normalizeSite(result: { pages?: SitePage[]; media?: MediaItem[]; business?: Record<string, unknown> | null }) {
  const pages = (result.pages ?? []).filter(page => page.id !== "site-messages").map(page => {
    ...
    const entries = page.entries.filter(entry => !isCodeManagedEntry(page.id, entry.key)).map(entry => {
  ...
  const media = (result.media ?? []).map(item => {
    if (!(item.key in mediaDefaults) || !/^\/(?!\/)/.test(item.url ?? "") || item.url.includes("..")) throw new Error(`Invalid website image: ${item.key}`);
